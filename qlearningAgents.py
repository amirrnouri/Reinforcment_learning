...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(actions) == 0:
          ...
          MaxActions = [action for action in actions if self.getQValue(state, action) == stateMaxQValue]
          Policy = random.choice(MaxActions)
        return Policy
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for i in featureVector.keys():
          Qvalue = Qvalue + self.weights[i] * featureVector[i]
        return Qvalue
    ...
    def update(self, state, action, nextState, reward):
        ...
        difference = (reward + self.discount*(NextStateQValue))- CurrentStateqValue
